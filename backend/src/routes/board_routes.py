...
async def get_details(id: str = Path(...)):
    ...
    if not board:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Not Found Board"
        )
...
async def update_board(
    id: str = Path(...), req_body: UpdateBoardValidation = Body(...)
) -> dict:
    board_controller = BoardController()
...
async def move_card_to_different_column(
    req_body: MoveCardToDifferentValidation = Body(...),
) -> dict:
    try:
        board_controller = BoardController()
